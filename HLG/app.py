...
def load_search_index() -> SearchIndex:
    ...
    geometric_vectors: list[np.ndarray] = []
    ...
    for row in rows:
        image_id, image_path, age, gender, race, color_hist, lbp, geometric_ratios = row
        color_vector = np.asarray(color_hist, dtype=np.float64)
        lbp_vector = np.asarray(lbp, dtype=np.float64)
        geometric_vector = np.asarray(geometric_ratios, dtype=np.float64)
        ...
        if color_vector.size != COLOR_HIST_LENGTH:
            ...
            continue
        if lbp_vector.size != LBP_LENGTH:
            LOGGER.warning("Bỏ qua image_id=%s vì độ dài LBP là %s.", image_id, lbp_vector.size)
            continue
        if geometric_vector.size != GEOMETRIC_RATIOS_LENGTH:
            LOGGER.warning("Bỏ qua image_id=%s vì độ dài geometric ratios là %s.", image_id, geometric_vector.size)
            continue
        ...
        geometric_vectors.append(geometric_vector)
...
def search_similar_images(query_path: Path, top_k: int = TOP_K) -> tuple[list[SearchResult], float]:
    ...
    g_norm = min_max_normalize(g_sim)

    # Trọng số: 0.30 Màu sắc, 0.20 LBP, 0.50 Tỷ lệ hình học
    total_similarity = (0.30 * h_norm) + (0.20 * l_norm) + (0.50 * g_norm)
    ...
    results = [SearchResult(
        ...
        float(h_norm[i]), float(l_norm[i]), float(g_norm[i])
...
class ChildFaceRetrievalApp(tk.Tk):
    ...
    def _configure_styles(self) -> None:
        ...
        for name, color in [("Color", "#3fb576"), ("Lbp", "#4f6ef7"), ("Geometric", "#5ac2f2")]:
            style.configure(
    ...
    def _create_result_card(self, parent: tk.Widget, res: SearchResult, rank: int) -> tk.Frame:
        ...
        self._add_similarity_row(body, "Kết cấu (LBP)", res.lbp_similarity, "Lbp")
        self._add_similarity_row(body, "Hình học (Tỷ lệ)", res.geometric_similarity, "Geometric")
        return card
