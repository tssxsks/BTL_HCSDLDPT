...
def create_tables(connection) -> None:
    ...
    CREATE TABLE IF NOT EXISTS image_features (
        ...
        lbp FLOAT8[] NOT NULL,
        geometric_ratios FLOAT8[] NOT NULL
    );
...
def build_record(image_path: Path, project_root: Path, extractor: FeatureExtractor) -> ETLRecord:
    ...
    return ETLRecord(
        ...
        lbp=features["lbp"],
        geometric_ratios=features["geometric_ratios"],
    )
...
def bulk_insert_records(connection, records: list[ETLRecord]) -> None:
    ...
    with connection.cursor() as cursor:
        ...
        execute_values(
            cursor,
            """
            INSERT INTO image_features (image_id, color_hist, lbp, geometric_ratios)
            VALUES %s;
            """,
            feature_rows,
